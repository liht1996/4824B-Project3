...
def build_model(embedding_dim, hidden_size, drop, sequence_length, vocabulary_size, embedding_matrix):
    inputs = Input(shape=(sequence_length,), dtype='int32')
    # inputs -> [batch_size, sequence_length]

    emb_layer = Embedding(input_dim=vocabulary_size, output_dim=embedding_dim, input_length=sequence_length, trainable = True)
...
def main(opt):
    ...
    if opt.mode == "train":
        st = time.time()
        print('Loading data')
        x_train, y_train, x_valid, y_valid, vocabulary_size, embedding_matrix = load_data("data", opt.embedding_dim, opt.debug)
        ...
        model = build_model(opt.embedding_dim, opt.hidden_size, opt.drop, sequence_length, vocabulary_size, embedding_matrix)
        print("Traning Model...")
