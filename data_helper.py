...
def load_data(data_path, EMBEDDING_DIM, debug=False):
    # get the data paths
    train_path = os.path.join(data_path, "valid.csv" if debug else "train.csv")
    valid_path = os.path.join(data_path, "valid.csv")
    vocab_path = os.path.join(data_path, "vocab.json")
    #model_path = os.path.join(data_path, "GoogleNews-vectors-negative300.bin")
    
                        
    train_data = read_data_from_file(train_path)
    valid_data = read_data_from_file(valid_path)
    #word2vec = KeyedVectors.load_word2vec_format(model_path, binary=True)
    W2Vmodel = Word2Vec(sentences=train_data, sg=1, hs=0, workers=4, size=EMBEDDING_DIM, min_count=3, window=10, sample=1e-3, negative=5, iter=6)
    ...
    # generate embedding matrix using pre-trained word2vec model
    ...
    return x_train, y_train, x_valid, y_valid, vocab_size, embedding_matrix
